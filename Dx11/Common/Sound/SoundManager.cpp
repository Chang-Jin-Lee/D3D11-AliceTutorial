...
#include <fmod.hpp>
#include "../Helper.h" // Utf8FromWString
#include <map>
#include <vector>
#include <algorithm>
#include <string>
...
namespace
{
	...
	// 2D Pan 테스트 채널
	FMOD::Channel* g_PanTestCh = nullptr;
	float g_PanTest = 0.0f;
...
bool Sound::Initialize()
{
	...
	// 3D 사운드 설정 (distanceFactor, dopplerScale, rolloffScale)
...
bool Sound::Play3DInstance(const std::wstring& instanceId, const std::wstring& soundKey, bool loop)
{
	if (!g_System) return false;
	...
	// 기존 있으면 정지
	Stop3DInstance(instanceId);

	FMOD::Channel* ch = nullptr;
	FMOD_RESULT r = g_System->playSound(it->second, nullptr, true, &ch); // paused start
	...
	// ===== 도플러 효과 끄기 =====
	// - FMOD 기본값: 리스너/소스 속도에 따라 도플러(피치) 자동 계산
	// - BGM/음악은 도플러를 거의 쓰지 않으므로, 채널 단위에서 완전히 꺼서
	//   캐릭터가 움직일 때 "테이프 빨리감기/찌직"처럼 들리는 문제를 제거한다.
...
void Sound::Stop3DInstance(const std::wstring& instanceId)
{
...
void Sound::Update3DInstance(const std::wstring& instanceId,
                             const XMFLOAT3& pos,
                             float volume01,
                             float minDist,
                             float maxDist)
{
	...
	FMOD_VECTOR p = ToFmod(pos);
	FMOD_VECTOR v = { 0,0,0 }; // 박스는 보통 고정(도플러 필요하면 여기에 속도 넣기)

	it->second.ch->set3DAttributes(&p, &v);
	it->second.ch->set3DMinMaxDistance(std::max(minDist, 0.001f),
	                                   std::max(maxDist, std::max(minDist, 0.001f)));
	it->second.ch->setVolume(std::clamp(volume01, 0.0f, 1.0f));
}

// ================= 테스트용 2D Pan 채널 =================

bool Sound::PlayPanTest2D(const std::wstring& soundKey, bool loop)
{
	if (!g_System) return false;
	...
	FMOD::Channel* ch = nullptr;
	FMOD_RESULT r = g_System->playSound(it->second, nullptr, true, &ch);
	if (!Check(r) || !ch) return false;
...
void Sound::SetPanTest2D(float panMinus1To1)
{
	g_PanTest = std::clamp(panMinus1To1, -1.0f, 1.0f);
	if (g_PanTestCh)
	{
		g_PanTestCh->setPan(g_PanTest);
	}
}
