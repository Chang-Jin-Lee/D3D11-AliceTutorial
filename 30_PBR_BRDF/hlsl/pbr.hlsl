...
// Shlick's approximation of the Fresnel factor. ÏµúÏÜåÍ∞? F0 , ÏµúÎ??Í∞íÏ?? 1.0,1.0,1.0
...
float4 main_ps(PixelShaderInput pin) : SV_Target
{
   ...
   
	{
		...
		float3 F  = fresnelSchlick(F0, max(0.0, dot(Lh, Lo)));  //ÏµúÏÜåÍ∞? F0 , ÏµúÎ??Í∞íÏ?? 1.0,1.0,1.0
		// Calculate normal distribution for specular BRDF.
        float D = ndfGGX(cosLh, max(0.01, roughness));		// ?ü¨?îÑ?ãà?ä§ 0 ?ù¥?êòÎ©? Í∞íÏù¥0?ù¥ ?êòÎØ?Î°? 0?ù¥Î©? ÏµúÏÜåÍ∞íÏÇ¨?ö©
		...
		// Í∏àÏÜç?Ñ±?ù¥ 1?ù¥Î©? Ïπ®Ìà¨Í∞? ?óÜ?úºÎØ?Î°? ?ëúÎ©¥ÏÇ∞????ù¥ ?ùº?ñ¥?ÇòÏß? ?ïä?ïÑ ?ëúÎ©¥ÏÇ∞????ùò ?Éâ?ÉÅ??? 0,0,0 ?úºÎ°? Ï≤òÎ¶¨
		// Í∏àÏÜç?Ñ±?ù¥ 0?ù¥Î©? Î∞òÏÇ¨?ùò ?ÇòÎ®∏Ï?? ÎπÑÏú®ÎßåÌÅº ?ëúÎ©¥ÏÇ∞????ùò ?Éâ?ÉÅ ?ëú?òÑ?ù¥ Í∞??ä•?ïò?ã§.
		// Í∏àÏÜç?Ñ±??? 1?îΩ????óê ?ï¥?ãπ?êò?äî ÎØ∏ÏÑ∏ ?ëúÎ©¥Ïùò Î∂ÑÌè¨Î•? ?ùòÎØ∏ÌïòÎØ?Î°? Î≥¥Í∞Ñ?ùÑ ?ÉÅ?öî?ïò?ó¨ Ï§ëÍ∞ÑÍ∞íÏùÑ Ï≤òÎ¶¨?ïú?ã§.
    ...
    if (useIBL.r > 0)		
	{
		...
		uint specularTextureLevels = querySpecularTextureLevels();  //  ?Öç?ä§Ï≥êÏùò ÏµúÎ?? LOD Í∞úÏàòÎ•? Íµ¨Ìïú?ã§.	
		float3 specularIrradiance = specularTexture.SampleLevel(defaultSampler, Lr, roughness * specularTextureLevels).rgb; 
		// ?ãú?Ñ†->?Ö∏Îß? Î∞òÏÇ¨Î≤°ÌÑ∞Î°? ?Éò?îåÎßÅÌïò?ó¨ Í±∞Ïπ†Í∏∞Î°ú LOD?†Å?ö©?êú specularIrradiance Î•? Íµ¨Ìïú?ã§.

		// Split-sum approximation factors for Cook-Torrance specular BRDF.
		float2 specularBRDF = specularBRDF_LUT.Sample(spBRDF_Sampler, float2(cosLo, roughness)).rg; // LUTÎ°? Îπ†Î•∏ Í∑ºÏÇ¨Í∞? float2 ?ùÑ Íµ¨Ìïú?ã§. Î¨¥Ïä®Í∞íÏùºÍπ?.
