...
Ownership during preparation: controller implements/tests the capture-key support in common/capture scripts and action fixture while Task2 owns only the app. The fresh Task3 integration worker consumes those changes and their recorded RED/GREEN evidence, owns solution/branding/manifest/README text and manifest fixture, and does not rewrite the controller's capture helpers. The Task3 review covers both portions together. Controller owns generated runtime PNG/GIF and visual QA; integration worker owns the generated info card/navigation.
