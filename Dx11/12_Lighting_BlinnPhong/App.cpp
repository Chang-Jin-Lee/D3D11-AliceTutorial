/*
* @brief  Blinn-Phong 조명과 스카이박스를 이용해 기본 광원/재질을 실시간으로 조작해 보는 예제입니다.
* @details
*   - 방향성 라이트(디렉셔널 라이트)와 머티리얼(ambient/diffuse/specular)을 UI로 변경해 보면서 Blinn-Phong 조명 결과를 관찰합니다.
*   - Skybox(큐브맵)를 이용해 환경을 둘러싸고, 현재 카메라 방향에 해당하는 큐브맵 면을 ImGui 창으로 미리 봅니다.
*   - 카메라는 공용 `Camera` 클래스와 `InputSystem`(RMB + WASD/E/Q)으로 제어하고, SystemInfomation으로 FPS/RAM/VRAM 정보를 출력합니다.
*/
