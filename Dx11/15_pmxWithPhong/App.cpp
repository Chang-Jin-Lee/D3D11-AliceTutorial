/*
* @brief  PMX 모델을 Phong/Blinn-Phong 조명과 스카이박스 환경맵으로 렌더링하는 예제입니다.
* @details
*   - 방향성 라이트와 머티리얼(ambient/diffuse/specular/reflect)을 조절하면서 PMX 캐릭터가 어떻게 조명/반사를 받는지 확인합니다.
*   - Skybox(큐브맵)를 배경으로 사용하고, 현재 카메라 방향에 해당하는 큐브맵 면을 ImGui 창으로 미리 확인할 수 있습니다.
*   - `PmxMangerWithoutTBN`, `LineRenderer`, `Skybox`를 함께 사용해 PMX 메시/라이트 방향/좌표축/오버레이 축을 단순한 코드로 시각화합니다.
*/
