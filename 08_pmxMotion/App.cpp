...
void App::OnUpdate(const float& dt)
{
	...
	XMMATRIX local0 = XMMatrixTranslation(m_RootPos.x, m_RootPos.y, m_RootPos.z); // 루트
	// 부모-자식 계층 world 계산: world = local * parentWorld
	XMMATRIX world0 = m_CenterMotion * local0; // 센터 본 모션을 루트에 적용(간단)
...
void App::OnRender()
{
	...
	// 7. 그리기
	{
		// 애니메이션 업데이트는 OnUpdate에서 수행됨. 여기선 본 CB만 업로드
		{
			D3D11_MAPPED_SUBRESOURCE mapped{};
			HR_T(m_pDeviceContext->Map(m_pBoneCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped));
			size_t nUpload = std::min<size_t>(m_BoneFinalMatrices.size(), 60);
			memcpy(mapped.pData, m_BoneFinalMatrices.data(), sizeof(XMMATRIX) * nUpload);
			m_pDeviceContext->Unmap(m_pBoneCB, 0);
		}
		{
			D3D11_MAPPED_SUBRESOURCE mapped{};
		...
		m_pDeviceContext->VSSetConstantBuffers(1, 1, &m_pBoneCB);
	...
	if (ImGui::Begin("Controls"))
	{
		...
		ImGui::SliderFloat("Speed", &m_PlaySpeed, 0.1f, 2.0f);
		float timeSec = m_PlayTime; if (ImGui::SliderFloat("Time (s)", &timeSec, 0.0f, 180.0f)) { m_PlayTime = timeSec; }
	}
	ImGui::End();
...
bool App::InitScene()
{
	...
	*/
	// 메시/본/가중치 병합(스키닝 전용: 노드 글로벌 변환을 적용하지 않고 바인드 포즈 기준으로 저장)
	std::unordered_map<std::wstring, int> boneNameToIndex;
	boneNameToIndex.reserve(256);
	auto toW = [](const aiString& s){ return Utf8ToW(s.C_Str()); };
	...
	for (unsigned mi = 0; mi < scene->mNumMeshes; ++mi)
	{
		...
		for (unsigned i = 0; i < mesh->mNumVertices; ++i)
		{
			...
			// bone indices/weights는 아래에서 채움
			m_ModelVertices.push_back(v);
			posRaw.push_back(p);
		}
		...
		for (unsigned bi = 0; bi < mesh->mNumBones; ++bi)
		{
			...
			std::wstring bname = toW(b->mName);
			int boneIndex;
			auto it = boneNameToIndex.find(bname);
			if (it == boneNameToIndex.end())
			{
				Bone nb{}; nb.name = bname; nb.offset = XMLoadFloat4x4((XMFLOAT4X4*)&b->mOffsetMatrix);
				// 노드 인덱스 매칭
				nb.nodeIndex = -1;
				for (size_t ni = 0; ni < m_NodeName.size(); ++ni)
				{
					if (m_NodeName[ni] == bname) { nb.nodeIndex = (int)ni; break; }
				}
			...
			for (unsigned w = 0; w < b->mNumWeights; ++w)
			{
				...
				size_t vid = baseIndex + vw.mVertexId;
				// 4개 슬롯에 채우기(간단 우선순위: 비어있는 슬롯부터)
				uint16_t* idx4 = (uint16_t*)m_ModelVertices[vid].boneIndex;
				float* w4 = &m_ModelVertices[vid].boneWeight.x;
				for (int k = 0; k < 4; ++k)
				{
					if (w4[k] == 0.0f)
					{
						uint16_t bi16 = (uint16_t)boneIndex;
						if (bi16 >= 60) { /* 넘는 본은 무시 */ break; }
						idx4[k] = bi16;
						w4[k] = vw.mWeight;
						break;
					}
	...
	*/
	...
	// 본 상수 버퍼 생성(b1)
	{
		...
		bd.ByteWidth = sizeof(XMMATRIX) * 60;
		HR_T(m_pDevice->CreateBuffer(&bd, nullptr, &m_pBoneCB));
		m_BoneFinalMatrices.assign(60, XMMatrixIdentity());
	}
...
// --- 스켈레톤/노드 계층 ---
void App::BuildNodeHierarchy(const aiScene* scene)
{
    // BFS로 노드 인덱스 부여
    m_NodeName.clear(); m_NodeParent.clear(); m_NodeLocalBind.clear();
    std::function<void(const aiNode*, int)> walk = [&](const aiNode* n, int parent){
        int idx = (int)m_NodeName.size();
        m_NodeName.push_back(std::wstring(n->mName.C_Str(), n->mName.C_Str()+strlen(n->mName.C_Str())));
        m_NodeParent.push_back(parent);
        m_NodeLocalBind.push_back(XMLoadFloat4x4((XMFLOAT4X4*)&n->mTransformation));
        for (unsigned i=0;i<n->mNumChildren;++i) walk(n->mChildren[i], idx);
    };
    walk(scene->mRootNode, -1);
    m_NodeLocalAnim = m_NodeLocalBind;
    m_NodeGlobal.resize(m_NodeName.size(), XMMatrixIdentity());
}

// --- VMD 간이 로더(본 키만, 선형 보간) ---
bool App::LoadVMD(const std::wstring& path)
{
    m_VmdTracks.clear();
    FILE* fp=nullptr; _wfopen_s(&fp, path.c_str(), L"rb"); if(!fp) return false;
    auto rd=[&](void* p,size_t s){ return fread(p,1,s,fp)==s; };
    char header[30]={0}; rd(header,30);
    // 본 키프레임 수
    uint32_t boneCount=0; rd(&boneCount,4);
    for (uint32_t i=0;i<boneCount;++i)
    {
        char nameSJIS[15]={0}; rd(nameSJIS,15);
        uint32_t frame; rd(&frame,4);
        float px,py,pz; rd(&px,4); rd(&py,4); rd(&pz,4);
        float qx,qy,qz,qw; rd(&qx,4); rd(&qy,4); rd(&qz,4); rd(&qw,4);
        unsigned char bez[64]; rd(bez,64); // 무시(선형)
        wchar_t wname[64]={0}; MultiByteToWideChar(932/*Shift-JIS*/,0,nameSJIS,-1,wname,64);
        		// 이름 정규화(예: 센터/Center/センタ? → 노드명과 매칭되게 간단 변환)
		std::wstring boneName = wname;
		if (boneName == L"センタ?" || boneName == L"Center" || boneName == L"センタ?" ) boneName = L"センタ?"; // 센터 표준화
        VmdKey k; k.frame=frame; k.pos={px,py,pz}; k.rot={qx,qy,qz,qw};
        m_VmdTracks[boneName].push_back(k);
    }
    fclose(fp);
    // 프레임 정렬 + 최대 프레임 계산 + 센터 본 캐시
    m_AnimMaxFrame = 0; m_CenterKeys.clear();
    for (auto& kv : m_VmdTracks)
    {
        auto& v = kv.second;
        std::sort(v.begin(), v.end(), [](const VmdKey&a,const VmdKey&b){return a.frame<b.frame;});
        if (!v.empty()) m_AnimMaxFrame = std::max<uint32_t>(m_AnimMaxFrame, v.back().frame);
        if (kv.first == L"センタ?") m_CenterKeys = v;
    }
    return true;
}

static void SampleTrack(const std::vector<App::VmdKey>& keys, float tFrame, XMFLOAT3& outP, XMFLOAT4& outQ)
{
...
void App::UpdateAnimation(float dt)
{
    m_PlayTime += dt * m_PlaySpeed;
    // 루프
    if (m_Loop && m_AnimMaxFrame > 0) {
        float maxSec = (float)m_AnimMaxFrame / 60.0f;
        if (m_PlayTime >= maxSec) m_PlayTime = fmodf(m_PlayTime, maxSec);
    }
    // 60fps 기준 프레임
    float tFrame = m_PlayTime * 60.0f;
    // 간단 모션: 센터 본만 샘플링해 루트 world에 적용
    m_CenterMotion = XMMatrixIdentity();
    {
        		if (!m_CenterKeys.empty())
		{
			XMFLOAT3 p; XMFLOAT4 q; SampleTrack(m_CenterKeys, tFrame, p, q);
			XMMATRIX T = XMMatrixTranslation(p.x, p.y, p.z);
			XMMATRIX R = XMMatrixRotationQuaternion(XMLoadFloat4(&q));
			m_CenterMotion = R * T;
		}
    }
    // 본 최종행렬 = global(nodeIndex) * offset
    size_t nBones = m_Bones.size();
    if (m_BoneFinalMatrices.size()<nBones) m_BoneFinalMatrices.resize(nBones);
    for (size_t bi=0; bi<nBones; ++bi)
    {
        int ni = m_Bones[bi].nodeIndex;
        XMMATRIX g = (ni>=0 && (size_t)ni<m_NodeGlobal.size()) ? m_NodeGlobal[ni] : XMMatrixIdentity();
        m_BoneFinalMatrices[bi] = m_Bones[bi].offset * g; // Assimp offset은 inverse bind pose
    }
}
