...
void App::OnRender()
{
	...
	if (m_->m_RenderMode == RenderMode::Model && !m_->m_Models.empty())
	{
		...
		for (auto& mdlPtr : m_->m_Models)
		{
			...
			// --------- 본 스켈레톤 디버그 박스 ---------
			if (mdlPtr->source == ModelSource::FBX && mdlPtr->shared && mdlPtr->shared->fbx && mdlPtr->shared->fbx->HasSkeleton())
			{
				std::vector<XMFLOAT4X4> globals;
				if (mdlPtr->animator.GetCurrentGlobalTransforms(globals) && !globals.empty())
				{
					const auto& nodeIndexOfName = mdlPtr->shared->fbx->GetNodeIndexOfName();
					const auto& boneNames = mdlPtr->shared->fbx->GetBoneNames();
					const auto& skel = mdlPtr->shared->fbx->GetSkeleton();

					// 라인용 CB: 월드=I로 두고, 좌표를 월드로 미리 변환해서 넣는다
					{
						ConstantBuffer lineCB = m_->m_ConstantBuffer;
					...
					// 라인 VS/IL 바인딩 저장 및 설정
					ID3D11VertexShader* prevVS2 = nullptr; m_->m_pDeviceContext->VSGetShader(&prevVS2, nullptr, nullptr);
					ID3D11InputLayout* prevIL2 = nullptr; m_->m_pDeviceContext->IAGetInputLayout(&prevIL2);
					m_->m_pDeviceContext->VSSetShader(m_->m_pLineVS, nullptr, 0);
					m_->m_pDeviceContext->IASetInputLayout(m_->m_pLineInputLayout);
					...
					XMMATRIX rotPitch = XMMatrixRotationX(XMConvertToRadians(mdlPtr->rotDeg.x));
					XMMATRIX rotRoll = XMMatrixRotationZ(XMConvertToRadians(mdlPtr->rotDeg.z));
					XMMATRIX Smdl = XMMatrixScaling(mdlPtr->scale.x, mdlPtr->scale.y, mdlPtr->scale.z);
					XMMATRIX Tmdl = XMMatrixTranslation(mdlPtr->pos.x, mdlPtr->pos.y, mdlPtr->pos.z);
					XMMATRIX Wmdl = Smdl * rotPitch * rotYaw * rotRoll * Tmdl;

					// 모델 대각선 길이 기반의 폴백 크기
					float fallbackR = 0.05f;
					if (mdlPtr->boundsValid)
					{
						XMFLOAT3 d = { mdlPtr->boundsMax.x - mdlPtr->boundsMin.x, mdlPtr->boundsMax.y - mdlPtr->boundsMin.y, mdlPtr->boundsMax.z - mdlPtr->boundsMin.z };
						float diag = sqrtf(d.x*d.x + d.y*d.y + d.z*d.z);
						fallbackR = (diag > 0.0f) ? (diag * 0.02f) : 0.05f;
					}

					XMFLOAT4 colBone = XMFLOAT4(0, 1, 1, 1); // 시안색
					// 각 본에 대해 박스 그리기
					for (size_t bi = 0; bi < boneNames.size(); ++bi)
					{
						auto itNode = nodeIndexOfName.find(boneNames[bi]); if (itNode == nodeIndexOfName.end()) continue;
						int nodeIdx = itNode->second; if (nodeIdx < 0 || nodeIdx >= (int)globals.size()) continue;
						XMMATRIX G = XMLoadFloat4x4(&globals[(size_t)nodeIdx]);
						XMMATRIX B = XMMatrixMultiply(Wmdl, G);
						// 위치
						XMFLOAT3 p;
						{ XMFLOAT4X4 b; XMStoreFloat4x4(&b, B); p = XMFLOAT3(b._41, b._42, b._43); }

                        // 간단 버전: 자식 길이 계산 없이 고정 크기 r만 사용 (월드축 정렬 박스)
                        float r = fallbackR;
                        XMFLOAT3 corners[8] = {
						...
						// 12 엣지 라인 출력 (월드 좌표 직접 전달)
						static const int E[24] = { 0,1,1,2,2,3,3,0, 4,5,5,6,6,7,7,4, 0,4,1,5,2,6,3,7 };
						for (int ei = 0; ei < 24; ei += 2)
						{
							m_->m_LineRenderer->DrawLine(m_->m_pDeviceContext, corners[E[ei+0]], corners[E[ei+1]], colBone, m_->m_pLineInputLayout, m_->m_pLineVS, m_->m_pPixelShader, m_->m_pConstantBuffer);
						}
					}

					// VS/IL 복원
					if (prevVS2) { m_->m_pDeviceContext->VSSetShader(prevVS2, nullptr, 0); prevVS2->Release(); }
					if (prevIL2) { m_->m_pDeviceContext->IASetInputLayout(prevIL2); prevIL2->Release(); }
				}
			}
...
void App::RenderControlPannel()
{
	...
	if (ImGui::Begin("Controls"))
	{
		...
		ImGui::Text("Camera");
		{
			...
			ImGui::SliderFloat("Camera Speed", &m_->m_camera.m_MoveSpeed, 1.0f, 500.0f, "%.1f");
...
void App::RenderModelPannel()
{
	...
	if (ImGui::Begin("Details"))
	{
		...
		if (m_->m_SelectedModelIdx >= 0 && m_->m_SelectedModelIdx < (int)m_->m_Models.size())
		{
			int i = m_->m_SelectedModelIdx;
			...
			{
				ImGui::TextUnformatted("Select a model in Scene Collection.");
			}
		}
		ImGui::End();
	}
...
void App::RenderSceneCollection()
{
	...
	if (ImGui::Begin("Scene Collection"))
	{
		...
		{
			UnloadModel();
			m_->m_RenderMode = RenderMode::None; // 시작/언로드시 아무것도 렌더 안함
		...
		for (int i = 0; i < (int)m_->m_Models.size(); ++i)
		{
			ImGui::PushID(i);
			...
			if (ImGui::SmallButton("Unload"))
			{
				...
				m_->m_Models.erase(m_->m_Models.begin() + i);
				if (m_->m_SelectedModelIdx >= (int)m_->m_Models.size()) m_->m_SelectedModelIdx = (int)m_->m_Models.size() - 1;
				ImGui::PopID();
				break;
			}
			ImGui::PopID();
