...
{
    // Shading mode quick path
    if (g_ShadingMode == 4) // TextureOnly
    {
        float4 textureColor = g_DiffuseMap.Sample(g_Sam, pIn.tex);
        return textureColor;
    }
    if (g_ShadingMode == 3) // Unlit
    {
        float4 textureColor = g_DiffuseMap.Sample(g_Sam, pIn.tex);
        float4 kd = textureColor * g_Material.diffuse;
        kd.a = textureColor.a * g_Material.diffuse.a;
        return kd;
    }
    ...
    float specTerm = (g_ShadingMode == 0) ? max(dot(reflectDir, eye), 0.0f) : pow(max(dot(reflectDir, eye), 0.0f), shininess);
    float specularScalar = specTerm * specGate * specTex;

    // ambient/diffuse/specular
    float4 ambient  = g_Material.ambient * g_DirLight.ambient;
    float4 diffuse  = (g_ShadingMode == 2) ? theta * float4(1,1,1,1) : theta * g_DirLight.diffuse;
