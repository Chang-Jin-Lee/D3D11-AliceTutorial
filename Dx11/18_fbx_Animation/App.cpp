...
void App::OnRender()
{
    ...
    if (m_->m_RenderMode == RenderMode::Model && m_->m_pModelVB && m_->m_pModelIB)
    {
        ...
        if (m_->m_ModelSource == ModelSource::FBX && m_->m_FbxManager.HasSkeleton())
        {
            ...
            // 본 팔레트 상수버퍼 b1 바인딩
            ID3D11Buffer* cbBones = m_->m_FbxManager.GetBoneConstantBuffer();
    ...
    m_->m_pDeviceContext->RSSetState(m_->RSNoCull);

    if (!(m_->m_RenderMode == RenderMode::Model && m_->m_ModelSource == ModelSource::FBX && m_->m_FbxManager.HasSkeleton()))
        m_->m_pDeviceContext->VSSetShader(m_->m_pVertexShader, nullptr, 0);
    ...
    ImGui::DragFloat3("Model Rotation (deg)", &m_->m_modelRotation.x, 1.0f, -360.0f, 360.0f, "%.1f");
        ...
        // FBX 본 트리와 애니메이션 UI
        if (m_->m_ModelSource == ModelSource::FBX && m_->m_RenderMode == RenderMode::Model)
        {
            if (ImGui::CollapsingHeader("FBX Skeleton & Animations", ImGuiTreeNodeFlags_DefaultOpen))
            {
                // 애니메이션 리스트
                if (m_->m_FbxManager.HasAnimations())
                {
                    const auto& names = m_->m_FbxManager.GetAnimationNames();
                    if (m_->m_UISelectedAnim < 0 || m_->m_UISelectedAnim >= (int)names.size()) m_->m_UISelectedAnim = m_->m_FbxManager.GetCurrentAnimationIndex();
                    ImGui::Text("Animations");
                    if (ImGui::BeginListBox("##AnimList", ImVec2(-FLT_MIN, 6 * ImGui::GetTextLineHeightWithSpacing())))
                    {
                        for (int i = 0; i < (int)names.size(); ++i)
                        {
                            bool sel = (i == m_->m_UISelectedAnim);
                            if (ImGui::Selectable(names[i].c_str(), sel))
                            {
                                m_->m_UISelectedAnim = i;
                                m_->m_FbxManager.SetCurrentAnimation(i);
                            }
                            if (sel) ImGui::SetItemDefaultFocus();
                        }
                        ImGui::EndListBox();
                    }
                    ImGui::Checkbox("Play", &m_->m_UIAnimPlaying);
                    m_->m_FbxManager.SetAnimationPlaying(m_->m_UIAnimPlaying);
                    double cur = m_->m_FbxManager.GetAnimationTimeSeconds();
                    double dur = m_->m_FbxManager.GetClipDurationSec(m_->m_FbxManager.GetCurrentAnimationIndex());
                    float curF = (float)cur, durF = (float)dur;
                    if (durF > 0.0f)
                    {
                        if (ImGui::SliderFloat("Time (s)", &curF, 0.0f, durF))
                        {
                            m_->m_FbxManager.SetAnimationTimeSeconds((double)curF);
                        }
                    }
                }
                else
                ...
                // 본 트리
                if (m_->m_FbxManager.HasSkeleton())
                {
                    const auto& skel = m_->m_FbxManager.GetSkeleton();
                    int root = m_->m_FbxManager.GetSkeletonRoot();
