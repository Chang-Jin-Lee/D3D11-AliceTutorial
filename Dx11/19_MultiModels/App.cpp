...
void App::OnUpdate(const float& dt)
{
    ...
    // Ïπ¥Î©î?ùº ?ûÖ?†•/?óÖ?ç∞?ù¥?ä∏?äî GameApp::m_Camera?óê?Ñú Ï≤òÎ¶¨?ïòÎØ?Î°?,
    // ?ó¨Í∏∞ÏÑú?äî world/view/projÎß? Ï±ÑÏõå?Ñú ?ÉÅ?àò Î≤ÑÌçºÎ°? ?ÑòÍπÅÎãà?ã§.
...
bool App::InitScene()
{
	...
	// ***********************************************************************************************
	// Ïπ¥Î©î?ùº ?Ñ§?†ï
	// Ïπ¥Î©î?ùº(View/Proj)Î°? ?ÉÅ?àò Î≤ÑÌçºÎ•? Ï§?ÎπÑÌï©?ãà?ã§ (GameApp::m_Camera ?Ç¨?ö©)
...
bool App::InitBasicEffect()
{
    ...
    HR_T(CompileShaderFromFile(L"19_BasicPS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
...
bool App::InitSkyBoxEffect()
{
	...
	ID3D10Blob* vertexShaderBuffer = nullptr;
    HR_T(CompileShaderFromFile(L"19_SkyBoxVS.hlsl", "VS", "vs_4_0", &vertexShaderBuffer));
	HR_T(m_->m_pDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	// Pixel Shader -------------------------------------
	ID3D10Blob* pixelShaderBuffer = nullptr;
    HR_T(CompileShaderFromFile(L"19_SkyBoxPS.hlsl", "PS", "ps_4_0", &pixelShaderBuffer));
	HR_T(m_->m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
...
void App::UnloadModel()
{
    ...
    // Í∞? ModelEntryÍ∞? Í∞?Ïß? Manager(FBX/OBJ/PMX)Í∞? ?Ç¥Î∂??óê?Ñú VB/IB/SRVÎ•? ?†ïÎ¶¨ÌïòÎØ?Î°?
    // ?ó¨Í∏∞ÏÑú?äî Ïª®ÌÖå?ù¥?ÑàÎß? ?†ïÎ¶¨Ìïú?ã§.
    m_->m_Models.clear();
    m_->m_RenderMode = RenderMode::None;
}
