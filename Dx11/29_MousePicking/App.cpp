...
void App::OnUpdate(const float& dt)
{
	...
	if (ImGui::IsKeyPressed(ImGuiKey_F6, true))
	{
		...
		if (m_SceneIndex == 0)
		{
			...
			m_SceneIndex = 1;
			// 씬 이미지 경로 변경
			m_->m_CurrentSceneImagePath = L"..\\Resource\\Image\\Public\\Comic\\02.png";
			LoadSceneImage(m_->m_CurrentSceneImagePath);
			// 팝업 표시
	...
	{
		if (LoadModelFromFile(L"..\\Resource\\fbx\\Public\\MyAlice\\Enemy\\AliceEnemy1.glb"))
		{
			int newIndex = (int)m_->m_Models.size() - 1;
			if (newIndex >= 0)
			{
				// 트랜스폼 배치: 스폰 횟수 기반 그리드
				const int perRow = 10;
				const float spacingX = 80.0f;
				const float spacingZ = 80.0f;
				int n = m_->m_SpawnTotal;
				int col = n % perRow;
				int row = n / perRow;
				...
				mdl.useInstanceMaterial = true;
				mdl.instanceMaterial.ambient = XMFLOAT4(
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					1.0f);
				mdl.instanceMaterial.diffuse = XMFLOAT4(
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					1.0f);
				mdl.instanceMaterial.specular = XMFLOAT4(
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					0.3f + 0.7f * ((float)(rand() % 100) / 100.0f),
					1.0f);
				// UI용 Scene Collection 등록
				auto mo = std::make_unique<ModelObject>(m_->m_Models[(size_t)newIndex]->modelName, newIndex);
				m_->m_Objects.push_back(std::move(mo));
				m_->PushLog("[OK] Spawned model instance (anim #1)");
				m_->m_SpawnTotal++;
			}
		}
	}
	// F8: 전부 삭제(길게 누르면 반복 동작)
	if (InputSystem::Instance && InputSystem::Instance->m_KeyboardStateTracker.IsKeyPressed(DirectX::Keyboard::Keys::F8))
	{
		// 모델 목록 비우기
		UnloadModel();
		// Scene Collection에서 모델 항목 제거 (큐브는 유지)
...
bool App::InitBasicEffect()
{
	...
	HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "main", "vs_5_0", &vertexShaderBuffer));
	...
	HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSNoTBN", "vs_5_0", &vsNoTBN));
	...
	HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSLine", "vs_5_0", &vsLine));
	...
	// FBX GPU 스키닝용 VS/IL 생성 (POSITION,NORMAL,TANGENT,BINORMAL,COLOR,TEXCOORD,BLENDINDICES,BLENDWEIGHT)
	{
		...
		HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSSkinned", "vs_5_0", &vsSkinned));
	...
	// Outline VS
	{
		...
		HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSOutline", "vs_5_0", &vsOutline));
		...
		HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSSkinnedOutline", "vs_5_0", &vsSkinnedOutline));
	...
	// Shadow VS (depth-only)
	{
		...
		HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSShadow", "vs_5_0", &vsShadow));
		...
		HR_T(CompileShaderFromFile(L"29_BasicVS.hlsl", "VSSkinnedShadow", "vs_5_0", &vsSkinnedShadow));
	...
	HR_T(CompileShaderFromFile(L"29_BasicPS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
	...
	// Outline 전용 Pixel Shader (PSOutline)
	{
		...
		HR_T(CompileShaderFromFile(L"29_BasicPS.hlsl", "PSOutline", "ps_4_0", &psOutline));
...
bool App::InitSkyBoxEffect()
{
	...
	ID3D10Blob* vertexShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"29_SkyBoxVS.hlsl", "VS", "vs_4_0", &vertexShaderBuffer));
	HR_T(m_->m_pDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	// Pixel Shader -------------------------------------
	ID3D10Blob* pixelShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"29_SkyBoxPS.hlsl", "PS", "ps_4_0", &pixelShaderBuffer));
	HR_T(m_->m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
