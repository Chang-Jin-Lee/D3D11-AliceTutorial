...
public:
	/*
	* @brief : D3D11 렌더링 파이프라인 핵심 객체들
	* @details :
	* 	- Device/Context/SwapChain/RTV : 기본 그리기 환경
	* 	- VS/PS/InputLayout : 셰이더와 입력 시그니처
	* 	- VB/IB : 메시 데이터(GPU 상주)
	* 	- ConstantBuffer : 셰이더에 바인딩하는 카메라/변환 행렬
	*/
	...
	/*
	* @brief : UI로 조절하는 카메라/월드 파라미터
	* @details :
	* 	- m_RootPos : 메시의 월드 평행이동
	* 	- m_CameraPos/Fov/Near/Far : 카메라 설정 (뷰/프로젝션에 반영)
	*/
	// ImGui 컨트롤 상태 변수
	...
	/*
	* @brief : FBX 로딩과 머티리얼/서브셋 상태
	* @details :
	* 	- m_ModelPath : 불러올 FBX 경로
	* 	- m_ModelVertices/Indices : 병합된 메시 데이터(VB/IB로 업로드)
	* 	- m_Subsets : 머티리얼별 드로우 범위(start/count/materialIndex)
	...
	* @brief : 장면 초기화/해제
	* @details :
	* 	- InitScene : FBX 로드 → 노드 계층 병합 → VB/IB/CB 생성 → 텍스처/샘플러 준비
