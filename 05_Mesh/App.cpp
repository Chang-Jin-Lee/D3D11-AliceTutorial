/*
* @brief : FBX Mesh 로더/렌더 예제 (텍스처/머티리얼 서브셋 지원)
* @details :
*   - 하니와(Haniwa) 메쉬 (정면 도식, 팔이 양옆으로 보이는 형태)
*
*         //        _____
*         //      /       \
*         //     |  o   o  |     <-- 눈
...
*
*		- 목적
*			FBX 형식의 메시 파일을 읽어 전체 노드 계층을 적용한 월드 공간 정점으로 병합하고,
*			머티리얼(주로 DIFFUSE)별 서브셋으로 나누어 텍스처까지 렌더링합니다.
*
*		- 리소스 구조(예시)
*			../Resource/haniwa/haniwa.fbx		: FBX 메시
*			../Resource/haniwa/*.png,*.jpg	: 머티리얼 텍스처(외부 파일)
*
*		- 렌더 파이프라인 개요
*			1) InitD3D
*				- 디바이스/스왑체인, RTV/DSV 생성 및 바인딩, Viewport 설정
*				- Depth 테스트 활성화(LESS), 래스터 Cull None (와인딩 이슈 대비)
*			2) InitEffect
*				- VS: TexVertexShader.hlsl (POSITION, TEXCOORD)
*				- PS: TexPixelShader.hlsl (텍스처 샘플)
*				- InputLayout: POSITION(float3), TEXCOORD(float2)
*			3) InitScene (FBX 로드 핵심)
*				- Assimp로 FBX 읽기 (Triangulate, JoinIdenticalVertices, ImproveCacheLocality,
*				  GenSmoothNormals, CalcTangentSpace, ConvertToLeftHanded)
*				- 머티리얼 텍스처 로딩
*					· 외부 파일: 모델 파일이 위치한 폴더를 기준으로 상대경로 결합
*					· 임베디드("*0" 등): 메모리에서 직접 텍스처 생성
*					· 실패 시 1x1 흰색 텍스처로 폴백
*				- 씬 루트노드부터 재귀 순회로 글로벌 변환을 정점에 적용하며 병합
*				- 서브셋(startIndex, indexCount, materialIndex) 기록
*				- AABB(min/max) → 중심 이동 + 스케일 정규화, 카메라 자동 설정
*				- 상수 버퍼(단일) 생성: world/view/proj
*			4) OnUpdate
*				- 단일 월드 변환(회전 Yaw + 평행이동 RootPos), view/proj 갱신
*			5) OnRender
*				- RTV/DSV 클리어 → IA(Topology, VB/IB, InputLayout) 설정
*				- VS/PS 바인딩 → 상수 버퍼 업로드
*				- 서브셋 순회: 머티리얼 SRV 바인딩 → DrawIndexed(indexCount, startIndex)
*				- 서브셋이 없다면 전체 DrawIndexed 수행
*			6) 종료
*				- 생성한 D3D 리소스 정리(Release)
*
*		- 좌표계/와인딩
*			DirectX 11은 왼손 좌표계(LH). Assimp ConvertToLeftHanded 사용.
*			FBX/수출 설정 차이에 따른 와인딩 뒤집힘 가능성 → Cull None 권장.
*/
...
* 	- InitScene 	: FBX 로드 → 서브셋/텍스처 준비 → VB/IB/CB 생성
*/
...
* 	- view/proj : UI에서 조정한 카메라 파라미터 반영
* 	- FPS 표시는 1초 간격으로 갱신
*/
...
/*
* @brief : 프레임 렌더(서브셋/머티리얼 텍스처 기반 드로우)
* @details :
* 	- OM/IA : 클리어, 토폴로지, VB/IB, 입력 레이아웃
* 	- VS/PS : 셰이더 바인딩, 상수 버퍼 업데이트
* 	- Draw  : 머티리얼 서브셋 순회 → SRV 바인딩 → DrawIndexed
*/
void App::OnRender()
{
...
* 	- DepthStencilState(LESS), Rasterizer(Cull None) 설정
* 	- Viewport 설정
*/
bool App::InitD3D()
{
 ...
 */
/*
* @brief : 장면 초기화(FBX 로드, 병합, 버퍼/텍스처 준비)
* @details :
* 	- FBX 로드 → 노드 변환 적용하며 정점 병합
* 	- 머티리얼 텍스처 로드(외부/임베디드), 실패 시 흰색 폴백
* 	- 서브셋(start/count/materialIndex) 기록
* 	- AABB 중심이동/스케일 정규화 → 카메라 자동 설정
* 	- VB/IB/CB 및 샘플러 생성
*/
bool App::InitScene()
{
	...
	*/
	...
	std::wstring baseDir = L"../Resource/haniwa/"; // 모델 폴더 기준(간단 처리)
...
* @details :
* 	- VS/PS 컴파일 → 객체 생성
* 	- POSITION/TEXCOORD 입력을 받는 InputLayout 생성
*/
bool App::InitEffect()
{
	// Vertex Shader -------------------------------------
	...
	*/
	...
	HR_T(CompileShaderFromFile(L"05_TexVertexShader.hlsl", "main", "vs_4_0", &vertexShaderBuffer));
	...
	*/
	...
	HR_T(CompileShaderFromFile(L"05_TexPixelShader.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
