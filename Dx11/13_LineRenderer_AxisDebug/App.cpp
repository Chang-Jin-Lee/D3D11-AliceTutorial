/*
* @brief  Blinn-Phong 조명 + 스카이박스 + 라인 렌더러로 좌표축/라이트 방향을 디버그 시각화하는 예제입니다.
* @details
*   - 조명/머티리얼 파라미터를 ImGui로 조절하면서, 큐브 위에 Blinn-Phong 하이라이트가 어떻게 보이는지 확인합니다.
*   - Skybox(큐브맵)를 배경으로 사용하고, 현재 카메라 방향에 해당하는 큐브맵 면을 별도 ImGui 창에 미리 봅니다.
*   - `LineRenderer`와 `Skybox` 유틸 클래스를 사용해 라이트 방향/월드 좌표축/화면 오버레이 축을 선형으로 그려줍니다.
*/
	...
	if (ImGui::Begin("Controls"))
	{
		// SkyBox 선택
		{
			...
			if (ImGui::Combo("SkyBox Choice", &cur, items, IM_ARRAYSIZE(items)))
			{
				...
				if (m_SkyBoxChoice == SkyBoxChoice::CubeMap)
...
bool App::InitBasicEffect()
{
	...
	HR_T(CompileShaderFromFile(L"13_BasicVS.hlsl", "main", "vs_4_0", &vertexShaderBuffer));
	...
	HR_T(CompileShaderFromFile(L"13_BasicPS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
...
bool App::InitSkyBoxEffect()
{
	...
	ID3D10Blob* vertexShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"13_SkyBoxVS.hlsl", "VS", "vs_4_0", &vertexShaderBuffer));
	HR_T(m_pDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	// Pixel Shader -------------------------------------
	ID3D10Blob* pixelShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"13_SkyboxPS.hlsl", "PS", "ps_4_0", &pixelShaderBuffer));
	HR_T(m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
