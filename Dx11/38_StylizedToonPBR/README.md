...
직접 만든 VRM 캐릭터를 GLB로 내보낸 `SampleModel.glb`를 같은 카메라·포즈·조명 조건에서 `PBR`, `Hybrid Toon-PBR`, `Split`으로 비교하는 Direct3D 11 예제입니다. PBR 입력을 유지하면서 명암 면, 머리카락 하이라이트, 림 라이트, 외곽선을 조절해 봅니다.

25번의 계단식 명암과 메시 외곽선에서 출발해, 30번의 PBR과 화면 공간 외곽선을 한 캐릭터에서 비교하는 단계입니다. 치마 레이스에는 중간 알파값이 있어, 불투명 의상과 반투명 표면의 차이도 함께 볼 수 있습니다.
...
| `Neon Contrast` | 중립 그림자와 순백색 키 라이트로 흰 의상을 중립으로 유지하면서 피부의 따뜻한 색과 명암 밴드를 분명하게 만듭니다. |
...
- `Skin`은 부드러운 스페큘러 응답을 사용합니다. 툰 경로에서는 실제 몸과 얼굴 피부에만 따뜻한 피치 응답을 적용해 ACES 뒤에도 얼굴·팔·다리가 흰 의상과 구분됩니다.
- `Hair`는 좁은 밴드 하이라이트와 짙은 재질 인지 외곽선을 사용합니다.
- `Cloth`는 넓고 조용한 스페큘러로 광택을 억제합니다.
- `SampleModel`의 재질 인덱스에는 프로젝트 내부의 명시적 프로필 오버라이드를 적용하고, 다른 재질 이름에는 `hair`, `face`/`skin`, `cloth`/`body` 분류를 폴백으로 사용합니다. 피부색 가중치는 이름까지 확인한 몸·얼굴 표면에만 별도로 지정합니다.
- 눈동자, 흰자, 입, 하이라이트, 눈썹은 BRDF 계산을 위해 `Skin` 프로필을 공유하지만 피부색 가중치는 받지 않습니다. 따라서 얼굴 디테일과 흰 의상은 원본 색을 유지합니다.
- 확산광은 `Band thresholds`와 `Band softness`로 제어하는 3영역 램프입니다. 중립 `Shadow tint`와 순백색 `Key tint`는 베이스 텍스처에 곱해지고, 실제 피부의 툰 베이스에만 별도의 따뜻한 응답이 적용됩니다.
...
| `N` / `Preset: Neon Contrast` | 중립 그림자·순백색 키의 강한 명암 대비 프리셋 |
...
프리셋을 다시 선택하면 관련 조명·밴드 파라미터가 해당 프리셋의 시작값으로 돌아갑니다. README 캡처 모드에서는 매니페스트의 스틸 지연 시점까지 결정론적 정지 포즈를 유지하고, 스틸 캡처 뒤 GIF 녹화가 시작되면 애니메이션을 재개합니다. 일반 실행에서는 내장 Idle 애니메이션이 계속 재생됩니다.
