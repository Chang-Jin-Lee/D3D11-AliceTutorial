/*
* @brief  노멀맵(법선 맵)을 이용해 벽돌 큐브에 Phong 조명을 적용해 보는 예제입니다.
* @details
*   - Diffuse/Normal/Specular 텍스처를 함께 사용해서, 동일한 메시 위에서 노멀맵이 어떻게 디테일을 만들어 내는지 확인합니다.
*   - Skybox(또는 단색 배경)를 선택할 수 있고, 현재 카메라 방향에 해당하는 큐브맵 면을 ImGui 창으로 미리 봅니다.
*   - `LineRenderer`와 `Skybox`를 이용해 라이트 방향/좌표축/오버레이 축을 단순한 코드로 그립니다.
*/
...
bool App::OnInitialize()
{
	...
	if (ReadmeCapture::IsEnabled())
	{
		...
		m_RotateCube = false;
		m_cubePos = XMFLOAT3(-2.8f, 0.0f, 0.0f);
		m_Camera.SetPosition(XMFLOAT3(-5.5f, 0.0f, -11.5f));
...
void App::OnUpdate(const float& dt)
{
	...
	XMMATRIX world0 = S * rotPitch * rotYaw * rotRoll * XMMatrixTranslation(m_cubePos.x, m_cubePos.y, m_cubePos.z); // 루트

	// ============================== 카메라 행렬 업데이트 ==============================
	// 카메라 입력/업데이트는 GameApp의 m_Camera에서 처리하므로,
	// 여기서는 world/view/proj만 캐시에 채워서 상수 버퍼로 넘깁니다.
...
void App::OnRender()
{
	...
	// ============================== 라이트 위치 마커 큐브 렌더 ==============================
	{
		...
		UINT dbgStride = sizeof(VertexTBN);
		UINT dbgOffset = 0;
		m_pDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	if (!ReadmeCapture::IsEnabled() && m_SkyBoxChoice != SkyBoxChoice::Off)
	{
		int face = 0;
...
bool App::InitBasicEffect()
{
	...
	HR_T(CompileShaderFromFile(L"16_BasicVS.hlsl", "VSLine", "vs_5_0", &vsLine));
    ...
    HR_T(CompileShaderFromFile(L"16_BasicPS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
