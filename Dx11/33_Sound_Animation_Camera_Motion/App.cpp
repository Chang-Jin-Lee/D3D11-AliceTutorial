/*
* @brief  : Sound + VMD Camera + Animation
* @details:
*/
...
bool App::InitBasicEffect()
{
	...
	HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "main", "vs_5_0", &vertexShaderBuffer));
	...
	HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSNoTBN", "vs_5_0", &vsNoTBN));
	...
	HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSLine", "vs_5_0", &vsLine));
	...
	// FBX GPU 스키닝용 VS/IL 생성 (POSITION,NORMAL,TANGENT,BINORMAL,COLOR,TEXCOORD,BLENDINDICES,BLENDWEIGHT)
	{
		...
		HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSSkinned", "vs_5_0", &vsSkinned));
	...
	// Outline VS
	{
		...
		HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSOutline", "vs_5_0", &vsOutline));
		...
		HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSSkinnedOutline", "vs_5_0", &vsSkinnedOutline));
	...
	// Shadow VS (depth-only)
	{
		...
		HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSShadow", "vs_5_0", &vsShadow));
		...
		HR_T(CompileShaderFromFile(L"33_BasicVS.hlsl", "VSSkinnedShadow", "vs_5_0", &vsSkinnedShadow));
	...
	HR_T(CompileShaderFromFile(L"33_BasicPS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
	...
	// Outline 전용 Pixel Shader (PSOutline)
	{
		...
		HR_T(CompileShaderFromFile(L"33_BasicPS.hlsl", "PSOutline", "ps_4_0", &psOutline));
...
bool App::InitSkyBoxEffect()
{
	...
	ID3D10Blob* vertexShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"33_SkyBoxVS.hlsl", "VS", "vs_4_0", &vertexShaderBuffer));
	HR_T(m_->m_pDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	// Pixel Shader -------------------------------------
	ID3D10Blob* pixelShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"33_SkyBoxPS.hlsl", "PS", "ps_4_0", &pixelShaderBuffer));
	HR_T(m_->m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
