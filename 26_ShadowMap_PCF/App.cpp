...
// 인덱스 버퍼를 스테이징으로 복사해 통계 계산 (삼각형 리스트 가정)
...
//////////////////////////////////////////////////////////////////////////
    ...
    float                         m_ShadowOrthoRadius = 20.0f; // 카메라 중심 반경(m)
    float                         m_ShadowNear = -50.0f;
...
bool App::OnInitialize()
{
	...
	// Ground plane (custom cube): large flat box for floor
	{
		StaticMeshData ground = StaticMesh::CreateBox(DirectX::XMFLOAT4(0.6f, 0.6f, 0.6f, 1.0f), 200.0f, 0.2f, 200.0f);
		ID3D11Buffer* vb = nullptr; ID3D11Buffer* ib = nullptr; int ic = 0;
		StaticMesh::AssignMemory(m_->m_pDevice, vb, ground);
		StaticMesh::AssignIndexMemory(m_->m_pDevice, ib, ground, ic);
		auto shared = std::make_shared<SharedModelData>();
		shared->source = ModelSource::Custom;
		shared->vb = vb; shared->ib = ib; shared->indexCount = ic;
		shared->stride = sizeof(VertexTBN);
		shared->subsets.clear(); shared->subsets.push_back({ 0u, (uint32_t)ic, 0u });
		shared->materialSRVs.clear(); shared->materialSRVs.push_back(nullptr); // fallback white will be used
		auto entry = std::make_unique<ModelEntry>();
		entry->modelName = L"GROUND";
		entry->source = ModelSource::Custom;
		entry->shared = shared;
		entry->pos = DirectX::XMFLOAT3(0.0f, -0.1f, 0.0f);
		entry->scale = DirectX::XMFLOAT3(1.0f, 1.0f, 1.0f);
		entry->rotDeg = DirectX::XMFLOAT3(0, 0, 0);
		entry->modelShading = ShadingMode::Lambert;
...
void App::OnUpdate(const float& dt)
{
    ...
    // Shadow light view-projection (directional, orthographic, camera-follow)
    {
        XMFLOAT3 camPosF = m_->m_camera.GetPosition();
        XMVECTOR camPos = XMLoadFloat3(&camPosF);
        XMVECTOR dirL = XMVector3Normalize(XMLoadFloat3(&m_->m_DirLight.direction));
        // Light looks along -dirL
        XMVECTOR fwd = XMVectorNegate(dirL);
        // Position the light a bit back along direction
        float backDist = 30.0f;
        XMVECTOR lightPos = XMVectorAdd(camPos, XMVectorScale(fwd, -backDist));
        XMVECTOR up = XMVectorSet(0,1,0,0);
        // Guard against parallel up
        ...
        float zn = (std::min)(m_->m_ShadowNear, m_->m_ShadowFar);
        float zf = (std::max)(m_->m_ShadowNear, m_->m_ShadowFar);
...
void App::OnRender()
{
    ...
    if (m_->m_ShadowEnabled && m_->m_RenderMode == RenderMode::Model && !m_->m_Models.empty() && m_->m_pShadowDSV)
    {
        // Save viewport
...
bool App::InitTexture()
{
	...
	{
		Microsoft::WRL::ComPtr<ID3D11Resource> res;
		HR_T(CreateWICTextureFromFile(m_->m_pDevice, m_->facePaths[i], res.GetAddressOf(), &m_->m_pCubeTextureSRVs[i]));
		res.Reset();
		HR_T(CreateWICTextureFromFile(m_->m_pDevice, m_->normalPaths[i], res.GetAddressOf(), &m_->m_pNormalSRVs[i]));
		res.Reset();
		HR_T(CreateWICTextureFromFile(m_->m_pDevice, m_->specularPaths[i], res.GetAddressOf(), &m_->m_pSpecularSRVs[i]));
		res.Reset();
	}
	return true;
}
...
bool App::InitBasicEffect()
{
    ...
    HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "main", "vs_5_0", &vertexShaderBuffer));
    ...
    HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSNoTBN", "vs_5_0", &vsNoTBN));
    ...
    HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSLine", "vs_5_0", &vsLine));
    ...
    // FBX GPU 스키닝용 VS/IL 생성 (POSITION,NORMAL,TANGENT,BINORMAL,COLOR,TEXCOORD,BLENDINDICES,BLENDWEIGHT)
    {
        ...
        HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSSkinned", "vs_5_0", &vsSkinned));
...
//////////////////////////////////////////////////////////////////////////
    // Outline VS
    {
        ...
        HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSOutline", "vs_5_0", &vsOutline));
        ...
        HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSSkinnedOutline", "vs_5_0", &vsSkinnedOutline));
    ...
    // Shadow VS (depth-only)
    {
        ...
        HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSShadow", "vs_5_0", &vsShadow));
        ...
        HR_T(CompileShaderFromFile(L"26_BasicVS.hlsl", "VSSkinnedShadow", "vs_5_0", &vsSkinnedShadow));
    ...
    HR_T(CompileShaderFromFile(L"26_BasicPS.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
	...
	// Outline 전용 Pixel Shader (PSOutline)
	{
		...
		HR_T(CompileShaderFromFile(L"26_BasicPS.hlsl", "PSOutline", "ps_4_0", &psOutline));
...
bool App::InitSkyBoxEffect()
{
	...
	ID3D10Blob* vertexShaderBuffer = nullptr;
    HR_T(CompileShaderFromFile(L"26_SkyBoxVS.hlsl", "VS", "vs_4_0", &vertexShaderBuffer));
	HR_T(m_->m_pDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	// Pixel Shader -------------------------------------
	ID3D10Blob* pixelShaderBuffer = nullptr;
    HR_T(CompileShaderFromFile(L"26_SkyBoxPS.hlsl", "PS", "ps_4_0", &pixelShaderBuffer));
	HR_T(m_->m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
...
//////////////////////////////////////////////////////////////////////////
		...
		{
			ImGui::Text("ShadowMap (Directional)");
...
void App::RenderModelPannel()
{
	...
	{
		UnloadModel();
		m_->m_RenderMode = RenderMode::None; // 시작/언로드시 아무것도 렌더 X
		m_->PushLog("[OK] Unloaded models");
	}
