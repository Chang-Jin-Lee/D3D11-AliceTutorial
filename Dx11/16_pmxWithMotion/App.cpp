...
void App::OnRender()
{
	...
	if (m_Pmx.HasMesh())
	{
		...
		XMMATRIX root = m_Pmx.HasRootMotion() ? m_Pmx.EvaluateRootMotion() : XMMatrixIdentity();
		XMMATRIX W = Sm * rotPitch * rotYaw * rotRoll * root * Tm;
		...
		// GPU 스키닝: slot0=vertex, slot1=influence
		ID3D11Buffer* vbs[2] = { vb, m_Pmx.GetInfluenceBuffer() };
		UINT strides[2] = { mStride, m_Pmx.GetInfluenceStride() };
		UINT offsets[2] = { mOffset, 0 };
		// 항상 두 슬롯을 바인딩 (slot1이 nullptr일 수 있음)
	    m_pDeviceContext->IASetVertexBuffers(0, 2, vbs, strides, offsets);
        ...
        // 본 행렬 상수버퍼(b1) 업로드 (재사용)
        {
			struct BonesCB { DirectX::XMFLOAT4X4 bones[256]; int numBones; float pad[3]; } bonesCB;
			//struct BonesCB { DirectX::XMFLOAT4X4 bones[64]; int numBones; float pad[3]; } bonesCB;
			//bonesCB.numBones = m_Pmx.FillBoneMatrices64(bonesCB.bones);
			bonesCB.numBones = m_Pmx.FillBoneMatrices256(bonesCB.bones);
            D3D11_MAPPED_SUBRESOURCE map{}; HR_T(m_pDeviceContext->Map(m_pBonesCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &map));
            memcpy(map.pData, &bonesCB, sizeof(BonesCB));
            m_pDeviceContext->Unmap(m_pBonesCB, 0);
            m_pDeviceContext->VSSetConstantBuffers(1, 1, &m_pBonesCB);
        }
	...
	// PMX/VMD 로드 및 정보/재생 컨트롤 UI
	if (ImGui::Begin("PMX/VMD"))
	{
		// PMX 로드
		if (ImGui::Button("Load PMX"))
		{
			...
			ofn.lpstrFilter = L"PMX Model (*.pmx)\0*.pmx\0All Files\0*.*\0\0";
			ofn.lpstrFile = file; ofn.nMaxFile = 1024;
			ofn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
			if (GetOpenFileNameW(&ofn))
			{
				// UI 표시용 UTF-8로 변환
				int lenA = WideCharToMultiByte(CP_UTF8, 0, file, -1, nullptr, 0, nullptr, nullptr);
				if (lenA > 0)
				{
					if ((int)sizeof(m_LastPmxPathA) < lenA) lenA = (int)sizeof(m_LastPmxPathA);
					WideCharToMultiByte(CP_UTF8, 0, file, -1, m_LastPmxPathA, lenA, nullptr, nullptr);
				}
				// 로드
				m_Pmx.Load(m_pDevice, std::wstring(file));
			}
		}

		if (m_Pmx.HasError())
		{
			std::wstring w = m_Pmx.GetLastError();
			ImGui::TextColored(ImVec4(1, 0.5f, 0.5f, 1), "%s", std::string(w.begin(), w.end()).c_str());
		}
		if (m_Pmx.HasMesh())
		{
			std::wstring p = m_Pmx.GetLoadedPath();
			// 표시용: UTF-16 -> UTF-8 변환
			int lenA = WideCharToMultiByte(CP_UTF8, 0, p.c_str(), -1, nullptr, 0, nullptr, nullptr);
			std::string pathA; pathA.resize((size_t)lenA ? (size_t)lenA - 1 : 0);
			if (lenA > 1) WideCharToMultiByte(CP_UTF8, 0, p.c_str(), -1, &pathA[0], lenA, nullptr, nullptr);
			ImGui::Text("Loaded: %s", pathA.c_str());
			ImGui::Text("Vertices: %u", (unsigned)m_Pmx.GetVertexCount());
			ImGui::Text("Indices: %d", m_Pmx.GetIndexCount());
			ImGui::Text("Subsets: %u, Materials: %u", (unsigned)m_Pmx.GetSubsetCount(), (unsigned)m_Pmx.GetMaterialCount());
		}

		ImGui::Separator();
		// VMD 로드
		// VMD 로드 버튼 (최근 선택 경로 사용)
		...
		if (ImGui::Button("Load VMD"))
		{
			...
			OPENFILENAMEW ofn{}; ofn.lStructSize = sizeof(ofn);
			ofn.hwndOwner = m_hWnd;
			ofn.lpstrFilter = L"MMD Motion (*.vmd)\0*.vmd\0All Files\0*.*\0\0";
			ofn.lpstrFile = file; ofn.nMaxFile = 1024;
			ofn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
			if (GetOpenFileNameW(&ofn))
			{
				...
				if (lenA > 0)
				{
					if ((int)sizeof(m_LastVmdPathA) < lenA) lenA = (int)sizeof(m_LastVmdPathA);
					WideCharToMultiByte(CP_UTF8, 0, file, -1, m_LastVmdPathA, lenA, nullptr, nullptr);
				}
				m_Pmx.LoadVmd(std::wstring(file));
				int t = 1;
			}
		}
		if (m_Pmx.HasVmd())
		{
			auto info = m_Pmx.GetVmdInfo();
			ImGui::Text("VMD: boneKeys=%u morphKeys=%u duration=%.2fs", info.numBoneKeys, info.numMorphKeys, info.durationSec);
			// 재생 컨트롤
			bool playing = m_Pmx.IsPlaying();
			if (!playing)
			{
				if (ImGui::Button("Play")) m_Pmx.Play();
				ImGui::SameLine();
				if (ImGui::Button("Stop")) m_Pmx.Stop();
			}
			else
			{
				if (ImGui::Button("Pause")) m_Pmx.Pause();
				ImGui::SameLine();
				if (ImGui::Button("Stop")) m_Pmx.Stop();
			}
			bool loop = m_Pmx.GetLoop();
			if (ImGui::Checkbox("Loop", &loop)) m_Pmx.SetLoop(loop);
			float speed = m_Pmx.GetPlaybackSpeed();
			if (ImGui::SliderFloat("Speed", &speed, 0.1f, 3.0f, "%.2fx")) m_Pmx.SetPlaybackSpeed(speed);
			float cur = m_Pmx.GetTime();
			if (ImGui::SliderFloat("Time", &cur, 0.0f, info.durationSec > 0 ? info.durationSec : 1.0f)) m_Pmx.SetTime(cur);
		}
		...
		{
			ImGui::Separator();
			ImGui::Text("Bones/Nodes Editor");
			static int selectedNode = 0;
			ImGui::BeginChild("NodeList", ImVec2(220, 240), true);
			size_t nodeCount = m_Pmx.GetNodeCount();
			for (size_t i = 0; i < nodeCount; ++i)
			{
				std::string nm = m_Pmx.GetNodeName(i);
				if (ImGui::Selectable(nm.c_str(), selectedNode == (int)i)) selectedNode = (int)i;
			}
			ImGui::EndChild();
			ImGui::SameLine();
			ImGui::BeginGroup();
			if (selectedNode >= 0 && (size_t)selectedNode < nodeCount)
			{
				DirectX::XMMATRIX cur = m_Pmx.GetLocalPoseMatrix((size_t)selectedNode);
				DirectX::XMFLOAT3 sclF{1,1,1}; DirectX::XMFLOAT4 rotQ{0,0,0,1}; DirectX::XMFLOAT3 posF{0,0,0};
				{
					DirectX::XMVECTOR s, r, t;
					DirectX::XMMatrixDecompose(&s, &r, &t, cur);
					DirectX::XMStoreFloat3(&sclF, s);
					DirectX::XMStoreFloat4(&rotQ, r);
					DirectX::XMStoreFloat3(&posF, t);
				}
				// 쿼터니언 → 오일러(deg)
				...
				t2 = t2 > 1.0f ? 1.0f : t2; t2 = t2 < -1.0f ? -1.0f : t2;
				float pitch = asinf(t2);
				float t3 = +2.0f * (rotQ.w * rotQ.z + rotQ.x * rotQ.y);
				float t4 = +1.0f - 2.0f * (ysqr + rotQ.z * rotQ.z);
				float yaw = atan2f(t3, t4);
				static float eul[3]; eul[0] = DirectX::XMConvertToDegrees(pitch); eul[1] = DirectX::XMConvertToDegrees(yaw); eul[2] = DirectX::XMConvertToDegrees(roll);
				float pos[3] = { posF.x, posF.y, posF.z };
				float scl[3] = { sclF.x, sclF.y, sclF.z };
				bool changed = false;
				changed |= ImGui::DragFloat3("Translate", pos, 0.01f);
				changed |= ImGui::DragFloat3("Scale", scl, 0.01f);
				changed |= ImGui::DragFloat3("Rotate (deg)", eul, 1.0f);
				if (changed)
				{
					DirectX::XMMATRIX rm = DirectX::XMMatrixRotationRollPitchYaw(DirectX::XMConvertToRadians(eul[0]), DirectX::XMConvertToRadians(eul[1]), DirectX::XMConvertToRadians(eul[2]));
					DirectX::XMMATRIX sm = DirectX::XMMatrixScaling(scl[0], scl[1], scl[2]);
					DirectX::XMMATRIX nm = sm * rm * DirectX::XMMatrixTranslation(pos[0], pos[1], pos[2]);
					m_Pmx.SetLocalPoseMatrix((size_t)selectedNode, nm);
					m_Pmx.RecomputeTransforms();
				}
			}
			ImGui::EndGroup();
		}
...
bool App::InitScene()
{
    ...
    // Bones constant buffer (b1)
    {
		//D3D11_BUFFER_DESC bd{}; bd.Usage = D3D11_USAGE_DYNAMIC; bd.ByteWidth = sizeof(DirectX::XMFLOAT4X4) * 64 + 16; bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER; bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		D3D11_BUFFER_DESC bd{}; bd.Usage = D3D11_USAGE_DYNAMIC; bd.ByteWidth = sizeof(DirectX::XMFLOAT4X4) * 256 + 16; bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER; bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        HR_T(m_pDevice->CreateBuffer(&bd, nullptr, &m_pBonesCB));
    }
...
bool App::InitBasicEffect()
{
	...
	D3D11_INPUT_ELEMENT_DESC layout[] = // 입력 레이아웃.
	{
		...
		{ "TEXCOORD", 1, DXGI_FORMAT_R32G32B32A32_UINT,   1, 0,  D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD", 2, DXGI_FORMAT_R32G32B32A32_FLOAT,  1, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 }
	};

	ID3D10Blob* vertexShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"16_BasicVS.hlsl", "main", "vs_5_0", &vertexShaderBuffer));
	...
	HR_T(CompileShaderFromFile(L"16_BasicPS.hlsl", "main", "ps_5_0", &pixelShaderBuffer));
...
bool App::InitSkyBoxEffect()
{
	...
	ID3D10Blob* vertexShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"16_SkyBoxVS.hlsl", "VS", "vs_4_0", &vertexShaderBuffer));
	HR_T(m_pDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	// Pixel Shader -------------------------------------
	ID3D10Blob* pixelShaderBuffer = nullptr;
	HR_T(CompileShaderFromFile(L"16_SkyBoxPS.hlsl", "PS", "ps_4_0", &pixelShaderBuffer));
	HR_T(m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
