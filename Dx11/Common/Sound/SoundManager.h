...
namespace Sound
{
	...
	// ================= 3D 사운드 =================
	// @brief 리스너(카메라/플레이어) 업데이트
	// @param pos : 리스너 위치
	// @param vel : 리스너 속도 (도플러 효과용, 보통 {0,0,0})
	// @param forward : 리스너가 바라보는 방향 (정규화된 벡터)
	...
	// @param soundKey : 사운드 키 (Load로 미리 로드되어 있어야 함)
	// @param loop : 루프 여부
	// @return 성공 여부
	...
	// @brief 3D 인스턴스 파라미터 업데이트(매 프레임 호출)
	// @param instanceId : 인스턴스 ID
	// @param pos : 소스 위치
	// @param volume01 : 볼륨 (0.0 ~ 1.0)
	// @param minDist : 최소 거리 (이 거리 이내에서는 볼륨이 최대)
	// @param maxDist : 최대 거리 (이 거리 밖에서는 볼륨이 0)
	...
	// ================= 테스트용 2D Pan 채널 =================
	// @brief 2D Pan 테스트 채널 재생
	bool PlayPanTest2D(const std::wstring& soundKey, bool loop = true);

	// @brief 2D Pan 테스트 채널 정지
	void StopPanTest2D();

	// @brief 2D Pan 테스트 채널 Pan 설정 (-1.0 ~ +1.0)
	void SetPanTest2D(float panMinus1To1);
}
